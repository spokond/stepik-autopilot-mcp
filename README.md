...
- Поддерживается практический блок Stepik `choice`: одиночный и множественный
  выбор.
...
- Остальные типы практических заданий пока показываются в плане как
  `unsupported` и не отправляются.
...
- `available` — доступные непройденные задания `choice`;
...
| `stepik_batch_commit` | Сохраняет или отправляет ответы `choice`. |
