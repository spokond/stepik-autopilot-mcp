...
- Поддерживаются `choice` (одиночный и множественный выбор), `string`, `number`,
  `sql`, `code`, `fill-blanks`, `matching` и `table`.
  Численные ответы передаются строкой в `reply.number` без потери точности,
  строковые — в `reply.text`; для `code`
  язык должен быть одним из языков конкретной попытки.
- Для `code` пакеты и `stepik_read` возвращают `code_templates`: исходный текст
  редактора для каждого языка, включая готовые списки и заготовки функций.
  Например, `code_templates["python3.12"]`. В старых запусках отсутствующие
  шаблоны загружаются чтением Stepik API, без создания попыток. Пробелы и переводы
  строк сохраняются; пустая карта означает отсутствие шаблонов, `null` — другой тип задания.
  После обновления переподключите MCP-клиент, чтобы он загрузил новые поля выдачи.
...
Для структурированных заданий пакет содержит `quiz_data` из конкретной попытки.
Эти данные и порядок вариантов сохраняются в SQLite и восстанавливаются после
перезапуска. Форматы поля `answer`:

| `kind` | Поля ответа | Правило |
|---|---|---|
| `number` | `"value": "5"` | Значение строкой, отправляется как `reply.number`. |
| `fill-blanks` | `"blanks": ["answer", "option"]` | По одному значению на каждый компонент `input`/`select`; компоненты `text` пропускаются. Для `select` нужно точное значение из `options`. |
| `matching` | `"ordering": [1, 0, 2]` | Для каждого `pairs[i].first` укажите индекс соответствующего `pairs[j].second`. Индексы с нуля, каждый используется один раз. |
| `table` | `"selected_columns": [[0], [1]]` | По списку индексов столбцов для каждой строки. Если `is_checkbox=false`, в каждой строке нужен ровно один индекс. |

Сервер проверяет длины, диапазоны индексов, повторы и режим выбора до отправки.
В табличном `reply.choices` используются имена строк и столбцов из попытки;
это другой формат, чем булев массив обычного `choice`.
Контракты сверены с официальным клиентом Stepik:
[модель Reply](https://github.com/StepicOrg/stepik-android/blob/master/model/src/main/java/org/stepik/android/model/Reply.kt),
[формирование matching](https://github.com/StepicOrg/stepik-android/blob/master/app/src/main/java/org/stepik/android/view/step_quiz_matching/ui/delegate/MatchingStepQuizFormDelegate.kt),
...
При потере ответа сначала выполните `resume`: автоматического повторения
неопределённой отправки нет. После частично выполненного запроса проверьте
состояния через `stepik_read`; новый запрос `retry` должен содержать только
оставшиеся `wrong` задания. Сохранённый после подготовки `leased` ответ можно
отправить обычным `submit`. Успешно завершённый запрос можно повторять с тем же
`request_id`: сервер вернёт прежние квитанции.

Чтобы включить ранее неподдерживаемые задания в существующий run, вызовите
`stepik_run_control` с `action="include"` и явным `explicit_step_ids`.
Сервер добавит отсутствующие поддерживаемые непройденные шаги этого курса как
`ready` и вернёт `added_items`. Этот вызов не создаёт попытки, не выдаёт пакет
и не меняет состояния существующих заданий. Теория, пройденные и уже включённые
шаги пропускаются. `stepik_read` теперь возвращает `step_id` и для `ready`.
...
| `stepik_batch_commit` | Сохраняет и отправляет поддерживаемые ответы; `retry` повторяет подтверждённые `wrong` числовые задания. |
| `stepik_results_collect` | Собирает оценки известных submissions. |
| `stepik_run_status` | Возвращает состояние run. |
| `stepik_run_control` | `pause`, `resume`, `cancel`; `resume` сверяет неопределённые отправки, `include` добавляет явные шаги без попыток. |
